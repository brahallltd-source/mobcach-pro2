...
export async function POST(req: Request) {
  try {
    ...
    if (!claimId || !["approve","reject"].includes(action)) return NextResponse.json({ message: "claimId and valid action are required" }, { status: 400 });
    ...
    if (index === -1) return NextResponse.json({ message: "Claim not found" }, { status: 404 });
    const claim = claims[index];
    claims[index] = { ...claim, status: action === "approve" ? "approved" : "rejected", reviewed_at: nowIso() };
    writeJsonArray(path, claims);
    if (action === "approve") creditWallet(claim.agentId, Number(claim.reward || 0), "bonus_claim_reward", { claimId: claim.id, track: claim.track, level: claim.level });
